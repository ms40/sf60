...
{
    QString ret = QString("Albatroscoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non ATR unit input in lnReqAmount, we generate the URI with ATR as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(AlbatroscoinUnits::format(AlbatroscoinUnits::ATR, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
